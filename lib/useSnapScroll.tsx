...
export default function createScrollSnap(
  ...
  if (
    settings.timeout &&
    (isNaN(settings.timeout) || typeof settings.timeout === 'boolean')
  ) {
    throw new Error(
  ...
  const timeout =
    settings.timeout && settings.timeout >= TIMEOUT_MIN
      ? settings.timeout
      : TIMEOUT_DEFAULT

  if (
    settings.duration &&
    (isNaN(settings.duration) || typeof settings.duration === 'boolean')
  ) {
    throw new Error(
  ...
  function checkScrollSpeed(value: number, axis: 'x' | 'y') {
    const clear = () => {
      lastScrollValue[axis] = undefined
    }
  ...
  function bindElement(element: HTMLElement) {
    ...
    snapLengthUnit = parseSnapCoordinatesValue(
      snapDestinationX,
      snapDestinationY
    )
  }
  ...
  function animationHandler() {
    ...
    if (
      scrollStart.y === target.scrollTop &&
      scrollStart.x === target.scrollLeft
    ) {
      // ignore timeout
      return
    }
  ...
  function isEdge(Coordinates: Coordinates) {
    return (
      (Coordinates.x === 0 && speedDeltaY === 0) ||
      (Coordinates.y === 0 && speedDeltaX === 0)
    )
  }
